...
namespace InstantMultiplayer.UnityIntegration.Controllers
{
    public class SyncInvocationEventMessageController : BaseMessageController<SyncInvocationEventMessage>
    {
        ...
        public SyncInvocationEventMessageController(PlayerConnectionsRepository playerConnectionsRepository)
        {
            _playerConnectionsRepository = playerConnectionsRepository;
        }

        public override void HandleMessage(SyncInvocationEventMessage message, TcpClient tcpClient)
        {
            for (int i = 0; i < 32; i++)
